...
param 'wanted', 'list what should be returned', :allows_multiple_values => true, 
  :lookup_method => lambda { %w|raw parsed stats timing| }, 
  :default_value => ['stats']
...
on_machine do |machine, params|
  ...
  if log_file.has_key?("parser") && log_file["parser"] && log_file["parser"] != ''
    parser = @op.list_parsers.select { |x| x['name'] == log_file['parser'] }.first
    raise "unknown parser '#{log_file['parser']}'" unless parser
  ...
  result['timing'] = timing if params['wanted'].include?('timing')
