...
run do |data|
  data.map do |line|
    ...
    # 1604078900 [30/Oct/2020:18:28:20 +0100] localhost ::1 - 200 683b 206micros - "GET / HTTP/1.1" "-" "curl/7.68.0"
    ...
    if result
      ...
      if entry
        ...
        if (entry[:x_forwarded_for]) then
          ...
          # TODO take the first part of the x_forwarded_for header instead?
          parts.reverse.each do |part|
            if part != "unknown" && part != "unknownn" && part != "-"
              source_ip = part
              break
            end
          end
        end
        entry[:source_ip] = source_ip || entry[:remote_ip]
      end
    else
